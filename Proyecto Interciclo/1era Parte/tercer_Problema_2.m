%Problema 3%
function   [sol2] = tercer_Problema_2(iteracionesMax, tolerancia);
format long
pkg load symbolic
warning('off','all');
syms x
...
% Funcion objetivo
funcion= x^3+4*x^2-40;
funcionG = @(t)  (10-(t^3)/4)^(1/2);
intervalo = [0, 4];
puntoInicio = 3;
...
tic
try 
  [sol2, datos,i_Newton] = Newton(funcion, intervalo , puntoInicio , tol =tolerancia, iter= iteracionesMax);
